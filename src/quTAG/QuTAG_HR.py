...
# Updated to be compatible with Linux
# Port Author: Faris Redza
# Tested with python3.10, Ubuntu 22.04 LTS
...
# os.add_dll_directory("C:\Program Files (x86)\QUTAG-HR-V1.0.1\quTAG_HR-Software_Python-examples-20220505\DLL_64bit\tdcbase.dll")
...
class QuTAG:
    ...
    def __init__(self):
        ...
        if os.name == 'nt':
            ...
            full_path = file_path + os.path.sep + os.path.join("LINUX_LIB")
            print(file_path)
        ...
        # the below line is what is breaking the python examples
        # 26_07 PB helped fix the line bellow, rather than: 
        #  
        # >> # load DLL 
        ...
        # load DLL
        if os.name == 'nt':
            self.qutools_dll = ctypes.windll.LoadLibrary(f'{full_path}\\{dll_name}')
        else:
            self.qutools_dll = ctypes.cdll.LoadLibrary(f'{full_path}/{dll_name}')
